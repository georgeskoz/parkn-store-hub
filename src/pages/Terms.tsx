...
import { TERMS_MARKDOWN_EN, TERMS_MARKDOWN_FR } from "@/content/legal/terms";

const Terms = () => {
  const { t, i18n } = useTranslation();
  const isFrench = i18n.language?.toLowerCase().startsWith("fr");
  const content = isFrench ? TERMS_MARKDOWN_FR : TERMS_MARKDOWN_EN;
  const title = t("legal.termsTitle");

  return (
    <div className="min-h-screen flex flex-col bg-background">
      <SEO title={title} description={title} path="/terms" />
      <Navbar />
